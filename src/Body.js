...
useEffect(() => {
  ...
  const posts = async() => {
    if (selected.length === 0) {
      const lenght = await axios.get('https://guarded-chamber-88701-7c7ee3d58107.herokuapp.com/get_length_bd');
      for (let i = 0; i < lenght.data.count_record; i++) {
        proms.push(axios.get(`https://guarded-chamber-88701-7c7ee3d58107.herokuapp.com/get_bd/${i}/`)); 
      }  
      ...
      setId(arr_id);
        ...
        const records = await axios.post('https://guarded-chamber-88701-7c7ee3d58107.herokuapp.com/get_length_bd_part', {selected});
